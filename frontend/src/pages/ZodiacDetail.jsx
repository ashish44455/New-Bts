  ...
  return (
    <>
      ...
      <div className="space-y-2">
        {/* Title: show ONLY selected language */}
        <div lang="en" className="lang-en font-display text-3xl font-semibold text-[color:var(--brand-text)] md:text-4xl">
          {titleEn}
        </div>
        <div lang="te" className="lang-te font-display text-2xl font-semibold text-[color:var(--brand-red)]">
          {item.te}
        </div>

        {/* Authority: show ONLY selected language */}
        <div className="mt-3 text-sm text-[color:var(--brand-text-muted)]">
          <div lang="en" className="lang-en">{ZODIAC_2026_2027.authorityEn}</div>
          <div lang="te" className="lang-te mt-1">{ZODIAC_2026_2027.authorityTe}</div>
        </div>

        {/* Year indicator */}
        <div lang="en" className="lang-en mt-3 text-sm font-semibold text-[color:var(--brand-text)]">
          Yearly Prediction – {ZODIAC_2026_2027.yearLabel}
        </div>
        <div lang="te" className="lang-te mt-3 text-sm font-semibold text-[color:var(--brand-text)]">
          వార్షిక ఫలితం – {ZODIAC_2026_2027.yearLabel}
        </div>
      </div>

      <Card className="rounded-3xl border-[rgba(201,162,77,0.35)] bg-white/35 p-6 shadow-[0_18px_55px_rgba(58,42,26,0.14)] backdrop-blur-[18px]">
        <div className="space-y-4 text-sm leading-relaxed text-[color:var(--brand-text-muted)] md:text-base">
          <div lang="en" className="lang-en">{item.textEn}</div>
          <div lang="te" className="lang-te">{item.textTe}</div>
        </div>

        <div className="mt-6 border-t border-[rgba(201,162,77,0.24)] pt-4 text-xs text-[color:var(--brand-text-muted)]">
          <div lang="en" className="lang-en">{ZODIAC_2026_2027.disclaimerEn}</div>
          <div lang="te" className="lang-te mt-1">{ZODIAC_2026_2027.disclaimerTe}</div>
        </div>
      </Card>
